...
router.post('/', async (req, res) => {
  try {
    ...
    const studioCheck = await db.query('SELECT slug FROM studios WHERE id = $1', [studioId]);
    ...
    if (existingCustomer.rows.length > 0) {
      ...
      const customerResult = await db.query(customerQuery, [
        studioId, customer.firstName, customer.email || null, customer.phone || null
      ]);
    ...
    do {
      ...
      const hashCheck = await db.query('SELECT id FROM invites WHERE short_hash = $1', [shortHash]);
    ...
    const { rows } = await db.query(inviteQuery, [studioId, customerId, shortHash, expiresAt]);
    const invite = rows[0];
