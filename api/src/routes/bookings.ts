...
router.patch('/:id/payment', async (req, res) => {
  try {
    ...
    const bookingCheck = await db.query('SELECT id, paid FROM bookings WHERE id = $1', [bookingId]);
    ...
    const { rows } = await db.query(updateQuery, [paymentDate, paidMethod, bookingId]);
