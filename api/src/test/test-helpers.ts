...
export async function cleanupDatabase() {
  ...
  await db.query('TRUNCATE TABLE bookings, children, invites, customers, slots, studios CASCADE');
}
...
export async function createTestStudio(studioData = testData.studio.valid) {
  // Generate unique slug for each studio creation
  const uniqueStudioData = {
    ...studioData,
    slug: `${studioData.slug}-${Date.now()}-${Math.random().toString(36).slice(2)}`
  };
...
export async function createTestSlot(studioId: string, slotData: any = testData.slot.adult) {
  const query = `
    INSERT INTO slots (
...
export async function createTestCustomer(studioId: string, customerData: any = testData.customer.withEmail) {
  const query = `
